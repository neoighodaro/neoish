    ...
    <Link href={`/posts/${slug}`}>
      <a title={title} className="w-full md:w-1/2 px-4 pb-12">
        ...
        <div className="relative px-4 -mt-16">
          <div className="p-6 bg-white dark-mode:bg-gray-800 rounded-lg shadow">
            ...
            <h2 className="my-1 font-medium text-xl text-gray-700 dark-mode:text-gray-300 leading-tight truncate">{title}</h2>
            <span className="flex text-gray-600 dark-mode:text-gray-500 text-sm">{truncateString(description, 80)}</span>
          </div>
        </div>
      </a>
    </Link>
